// @ts-nocheck
// Supabase Edge Function: suggestions
// Mirrors POST /api/suggestions from server.js
...
serve(async (req) => {
  ...
  try {
    ...
    if (!resumeText || !jobDetails) return new Response(JSON.stringify({ error: 'Missing resumeText or jobDetails' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
...
`;
    ...
    return new Response(JSON.stringify({ suggestions: String(text) }), { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
  } catch (err) {
    return new Response(JSON.stringify({ error: 'Suggestions generation failed', details: String(err?.message || err) }), { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
  }
});
